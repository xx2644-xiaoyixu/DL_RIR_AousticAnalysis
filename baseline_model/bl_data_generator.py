...
class IRDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = np.load(feat_path)   # 期望 shape: (2, T, F)

        if x.ndim != 3 or x.shape[0] != 2:
            raise ValueError(f"{filename} shape should be (2, T, F), but got {x.shape}")

        y = row[['DRR', 'C80', 'RT60']].values.astype(np.float32)
...
class DataGenerator(object):
    def __init__(
            self, shuffle=True, is_eval=False
    ):
